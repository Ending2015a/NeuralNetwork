...
std::vector<std::vector<double>> Generate_TestCase() {
	std::vector<std::vector<double>> testcase;
...
int main(void) {
	...
	std::vector<std::vector<double>> tc = Generate_TestCase();

	
	std::vector<std::vector<double>> o = nn << tc;

	for (size_t i = 0; i < o.size(); i++) {
		fout << (int)std::distance(o[i].begin(), std::max_element(o[i].begin(), o[i].end())) << std::endl;
	}
